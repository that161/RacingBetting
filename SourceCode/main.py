...
def SaveGame(lSite, money, setUnlocked):
    data.iloc[lSite, 2] = money
    for i in range(3,8):
        data.iloc[lSite, i] = setUnlocked[i - 3]
...
def loadGame():
    ...
    setUnlocked = [0, 0, 0, 0, 0]
    for i in range(3,8):
        setUnlocked[i - 3] = int(data.iloc[site, i])
    lSite = site
    return lSite, username, password, money, setUnlocked
...
def signUpAndLoad(username, password):
    ...
    quantity += 1
    setUnlocked = [1, 0, 0, 0, 0]
    data.iloc[0,8] = quantity
    for i in range(4,8):
        data.iloc[quantity - 1, i] = 0
    data.iloc[quantity - 1, 3] = 1
    data.iloc[quantity - 1, 2] = 5000
    money = 5000
    lSite = quantity - 1
    data.to_csv(database, index = False)
    return lSite, username, password, money, setUnlocked
...
def mainMenu(lSite, money, characterSet, username, setUnlocked):
    ...
    while Running:
        ...
        if exitButton.collidepoint(dx, dy):
            if clicked:
                SaveGame(lSite, money, setUnlocked)
                exitConfirmScreen()
        ...
        if changeSetButton.collidepoint(dx, dy):
            ...
            if clicked:
                characterSet = changeSetScreen(characterSet, setUnlocked)
        ...
        if logOutButton.collidepoint(dx, dy):
            if clicked:
                menuSound.stop()
                SaveGame(lSite, money, setUnlocked)
                lSite, username, password, money, setUnlocked = loginscreen()
                logOut = True
...
def changeSetScreen(selectedSet, setUnlocked):
    running = True
    clicked = False
    lockedSet = False
    frame = 0
    notificationArea = (445, 445, 400, 30)
    while running:
        DISPLAYSURFACE.blit(changeSet, (0,0))
        draw_text(str(setUnlocked), font, (0,0,0), DISPLAYSURFACE, 620, 700)

        if lockedSet and frame < 24:
            pygame.draw.rect(DISPLAYSURFACE, (0,0,0), notificationArea)
            draw_text("YOU HAVEN'T UNLOCK THIS SET YET", font, (255, 255, 255), DISPLAYSURFACE, 450, 450)
            t.sleep(0.0125)
            frame += 1
        else:
            frame = 0
...
def main():
    lSite, username, password, money, setUnlocked = loginscreen()
    mainMenu(lSite, money, characterSet, username, setUnlocked)
